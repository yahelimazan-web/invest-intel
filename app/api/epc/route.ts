...
const EPC_API_URL = "https://epc.opendatacommunities.org/api/v1/domestic/search";
...
export async function GET(request: NextRequest) {
  ...
  if (!apiKey || apiKey === "your_epc_api_key_here") {
    console.warn("[EPC API Route] EPC_API_KEY not configured or is placeholder");
    return NextResponse.json(
      { 
        error: "API key not configured", 
        code: "AWAITING_API_KEY",
        message: "ממתין להזנת מפתח API במערכת",
        instructions: "יש להירשם ב-epc.opendatacommunities.org ולהזין את המפתח ב-.env.local"
      },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    // Debug logging
    console.log(`[EPC API Route] Auth format: ${apiKey.includes(':') ? 'email:key' : 'key:empty'}`);
    console.log(`[EPC API Route] Credentials: ${authCredentials.substring(0, 25)}...`);
    console.log(`[EPC API Route] Base64 header: ${authHeader.substring(0, 40)}...`);
    ...
    console.log(`[EPC API Route] Success: ${data.rows?.length || 0} EPC records found`);
