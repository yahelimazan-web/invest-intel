...
async function fetchBOEData(): Promise<{ rate: number; previous: number; lastUpdated: string }> {
  try {
...
async function fetchONSInflation(): Promise<{ cpi: number; cpih: number; rpi: number; lastUpdated: string }> {
  try {
    // ONS API for CPI
    const response = await fetch(
    ...
    if (response.ok) {
      ...
      return {
        ...
        lastUpdated: latestMonth?.date || new Date().toISOString().split("T")[0],
      };
    }
  } catch (error) {
...
async function fetchUKHPI(): Promise<{ average: number; annualChange: number; monthlyChange: number; lastUpdated: string }> {
  try {
    ...
    if (response.ok) {
      ...
      if (results && results.length >= 1) {
        ...
        return {
          ...
          lastUpdated: latest.date?.value || new Date().toISOString().split("T")[0],
        };
      }
    }
  } catch (error) {
...
async function fetchGDPData(): Promise<{ quarterly: number; annual: number; lastUpdated: string }> {
  try {
    const response = await fetch(
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[BoE] Bank Rate:", boeData.rate, "% (Previous:", boeData.previous, "%)");
    console.log("[ONS] Inflation CPI:", inflationData.cpi, "%");
    console.log("[UKHPI] Average Price: £", hpiData.average, "Annual Change:", hpiData.annualChange, "%");
