...
import { sendMonthlyInsightsEmail, sendTestEmail } from "../../lib/email-service";
  ...
  try {
    ...
    if (action === "monthly") {
      if (!userEmail || !userName || !insights) {
        return NextResponse.json(
          { success: false, error: "userEmail, userName, and insights are required" },
          { status: 400 }
        );
      }

      const result = await sendMonthlyInsightsEmail(userEmail, userName, insights);
      return NextResponse.json(result);
    }
